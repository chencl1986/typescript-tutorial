...
/* 
声明一个Greeter类，包含3个成员：greeting属性、greet方法和构造函数。
*/
...
/* 
类从基类中继承属性和方法。Dog是一个派生类（又称作子类），通过extends关键字，派生自Animal基类（又称作超类）。
...
但对比private或protected成员的类型时。

若其中一个类型里包含一个private成员，同时另一个类型中也存在这样的private成员，而且它们来自于同一处声明，这两个类型才是兼容的。
...
/* 
可以在构造函数中，使用参数属性直接为类定义属性及其类型。

在实例化时，就会为创建相应类型的属性，以此将声明和复制合并到一起。

可以接受private、public、protected、readonly修饰符。
*/

{
  class Animal {
    public publicName: string
    public constructor(private name: string, public age: number, protected weight: string, readonly speed: number) {
      this.publicName = name
    }
    move(distanceInMeters: number) {
      console.log(`${this.name} (age: ${this.age}, weight: ${this.weight}, speed: ${this.speed}) moved ${distanceInMeters}m.`)
    }
  }
  const animal = new Animal('Snake', 8, '6kg', 10)
  animal.move(10)
}
...
下面的例子中，代码会先判断passcode是否正确，若为正确则允许修改fullName的值。
*/
...

{
  class Grid {
    ...
    calculateDistanceFromOrigin(point: { x: number; y: number; }) {
      ...
      console.log(this.origin)  // 错误，属性不存在于实例上，无法访问。
...
/* 
在TypeScript中声明一个类时，首先声明了类的实例的类型。
...
Greeter将被赋值为构造函数，用new执行后，将会得到一个类的实例。

构造函数包含了类的所有静态属性。

可以说，类具有实例部分和静态部分两个部分。
*/
...
greeter1与上面的例子一样，是由Greeter类实例化出的对象。

而变量GreeterMaker保存了Greeter类，同时通过typeof Greeter，获取类Greeter的类型，而不是实例类型。

准确的说typeof获取的是Greeter标识符的类型，也就是构造函数的类型。

这个类型包含了类的所有静态类型和构造函数。

因此也可以通过new GreeterMaker，创建Greeter的实例。
*/
