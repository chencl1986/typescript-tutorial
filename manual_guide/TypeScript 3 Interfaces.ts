export default function (): void {
  ...
  // 接口的作用就是为类型命名和为你的代码或第三方代码定义契约，即定义变量的数据格式。

  // 1. 简单的接口例子
  // 定义一个接口ILabelledValue，其包含属性label，类型为string
  // 传入一个变量时，编译器会检查定义的属性是否存在，同时允许其他属性传入，但使用其他属性时会报错。
  // 直接传入参数时，编译器会检查参数属性是否存在
  // 接口的检查不强制要求顺序一致。
  ...
  // interface ITest extends ISquareConfig {
  //   test: string
  // }
  ...
  // const mySquare = createSquare(testObj)
  // const mySquare = createSquare({ color: 'black' })
  // const mySquare = createSquare({ color: 'black', test: 'test' }) // 直接传如未定义参数会报错
  ...
  console.log(p1)
  // p1.x = 5 // 报错，x为只读属性
  const p2: number[] = [0, 1, 2]  // 普通类型及时用const定义，内部状态还是可以被更改
  p2[0] = 1
  console.log(p2)
  ...
  // a = ro // error!
  // a = ro // 不可将只读类型赋值给普通类型
  a = ro as number[] // 可以将类型断言重写后，赋值给a
  ...
  const mySquare1 = createSquare({ colour: 'red', width: 100 })  // 报错，传入了未定义的属性
  const mySquare2 = createSquare(testObj)  // 不报错，通过定义变量后，改为传入变量，不会报错
  const mySquare3 = createSquare({ colour: 'red', width: 100 } as ISquareConfig)  // 不报错，进行了类型断言，也不会报错
  ...
  // 使用接口定义函数类型，在文档中OK，但会被TSLint阻止，需要使用类型别名定义。
